...
def get_vote(tree,row):
    key = next(iter(tree))
    while key != 'val':
        tree = tree[key][row[key]]
        key = next(iter(tree))
    return tree[key]

def predict_hold(df,forest):
    a0p0 = 0 
    a0p1 = 0
    a1p0 = 0
    a1p1 = 0
    for index, row in df.iterrows():
        ...
        if(zero_ct > one_ct):
            ...
            guess = 1
        if(guess==0 and row["category"]==0):
            a0p0 +=1
        elif(guess==0 and row["category"]==1):
            a1p0 +=1
        elif(guess==1 and row["category"]==0):
            a0p1 +=1
        elif(guess==1 and row["category"]==1):
            a1p1 +=1
...
for col in non_cts:
    df_test[col].replace([" ?"], [df[col].mode()], inplace = True)

tree_num = 9
forest = []
for i in range(1,tree_num+1):
    df = df.sample(frac=1).reset_index(drop=True)
    forest.append(build_tree(df[:10000], get_attr_feature(df), 0))

a0_p0, a0_p1, a1_p0, a1_p1 = predict_hold(df_test,forest)
